...
// type_constructor_ptr_tag_snmp_oid_s
// 
void type_constructor_ptr_tag_snmp_oid_s(signed int depth_ptr_tag_snmp_oid_s, snmp_oid_t **result_ptr_tag_snmp_oid_s);
...
// __GOTO_HARNESS::has_seen_tag_snmp_oid_s
// file __GOTO_HARNESSharness.c
signed int has_seen_tag_snmp_oid_s=0;
...
void harness(void)
{
  ...
  type_constructor_ptr_tag_snmp_oid_s(0, &oid);
  snmp_ber_decode_oid(snmp_packet, oid);
  free(snmp_packet);
  free(oid);
}
...
void type_constructor_ptr_tag_snmp_oid_s(signed int depth_ptr_tag_snmp_oid_s, snmp_oid_t **result_ptr_tag_snmp_oid_s)
{
  if(depth_ptr_tag_snmp_oid_s >= max_depth && has_seen_tag_snmp_oid_s == 1)
    *result_ptr_tag_snmp_oid_s = ((snmp_oid_t *)NULL);

  else
  {
    __CPROVER_bool should_recurse_nondet$1;
    if(!(depth_ptr_tag_snmp_oid_s >= min_depth) || should_recurse_nondet$1)
    {
      signed int has_seen_prev_tag_snmp_oid_s=has_seen_tag_snmp_oid_s;
      has_seen_tag_snmp_oid_s = 1;
      snmp_oid_t *local_result$1=malloc(72u);
      type_constructor_tag_snmp_oid_s(depth_ptr_tag_snmp_oid_s + 1, &(*local_result$1));
      *result_ptr_tag_snmp_oid_s = local_result$1;
      has_seen_tag_snmp_oid_s = has_seen_prev_tag_snmp_oid_s;
    }
...
void type_constructor_tag_snmp_oid_s(signed int depth_tag_snmp_oid_s, snmp_oid_t *result_tag_snmp_oid_s)
{
  ...
  type_constructor_ptr_uint(depth_tag_snmp_oid_s, &result_tag_snmp_oid_s->test);
}
